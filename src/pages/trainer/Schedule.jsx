...
import { ScheduleForm } from '../../components/trainers/ScheduleForm';
import { ScheduleDisplay } from '../../components/trainers/ScheduleDisplay';
    ...
    if (user) {
      try {
        for (const entry of scheduleEntries) {
          ...
          setSchedules([...schedules, { ...entry, id: docRef.id, userEmail: user.email }]);
        }
  ...
  const handleUpdateSchedule = async (updatedSchedule) => {
    try {
      ...
      setSchedules(schedules.map(schedule => 
        schedule.id === updatedSchedule.id ? { ...schedule, ...updatedSchedule } : schedule
      ));
      ...
      content={
        <div className="p-5">
          ...
          {showModal && (
           ...
           <ScheduleDisplay schedules={schedules} onDeleteSchedule={handleDeleteSchedule} onUpdateSchedule={handleUpdateSchedule} />
        </div>
      }
    />
  );
